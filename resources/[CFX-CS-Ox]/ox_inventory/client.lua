...
require 'modules.custom_items.client'
...
function client.openInventory(inv, data)
    ...
    if inv == 'player' and data ~= cache.serverId then
        ...
        if not targetCoords or #(targetCoords - GetEntityCoords(playerPed)) > 1.8 or not Player(serverId).state.canSteal then
            return lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access') })
        end
    end
    ...
    elseif invOpen ~= nil then
        if inv == 'policeevidence' then
            local prefix = 'pd'
            local title = locale('police_evidence') or 'Police Evidence Locker'

            if type(data) == 'table' then
                prefix = data.prefix or data.department or prefix
                title = data.label or title
                data = data.locker or data.id
            end

            if not data then
                local input = lib.inputDialog(title, {
                    { label = 'Evidence Locker Code', description = 'Enter case code or locker number', type = 'input', required = true, icon = 'lock' }
                })
            ...
            data = { locker = data, prefix = prefix }
        end
...
lib.callback.register('ox_inventory:usingItem', function(data, noAnim)
	...
	if item and usingItem then
		...
		-- Keep current weapon equipped during food/drink/item anims (upper-body only)
		local progressAnim = item.anim or item.scenario
		if currentWeapon and type(progressAnim) == 'table' then
			progressAnim = {
				dict = progressAnim.dict,
				clip = progressAnim.clip,
				flag = progressAnim.flag or 49,
				blendIn = progressAnim.blendIn,
				blendOut = progressAnim.blendOut,
				duration = progressAnim.duration,
				playbackRate = progressAnim.playbackRate,
				lockX = progressAnim.lockX,
				lockY = progressAnim.lockY,
				lockZ = progressAnim.lockZ,
				scenario = progressAnim.scenario,
				playEnter = progressAnim.playEnter,
			}
		end
	...
	if canUseItem(data.ammo and true) then
		...
		elseif currentWeapon then
			...
			elseif data.component then
				...
				-- Allow using any consumable/item while holding a weapon
				useItem(data)
			end
			...
			if not canOpenInventory() then
				...
				if not currentInventory.ignoreSecurityChecks then
					...
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > maxDistance or not Player(currentInventory.id).state.canSteal then
							client.closeInventory()
