...
local function newItem(data)
	...
	-- Food / drink / consumables with status are usable while holding a gun
	if data.allowArmed == nil and clientData and clientData.status then
		data.allowArmed = true
	end

	if isServer then
        ---@cast data OxServerItem
        serverData = data.server
		-- Keep client.status on the server so consumables can restore hunger/thirst
		-- through SecureApplyItemStatus (esx_status client events are not used).
		if clientData and clientData.status then
