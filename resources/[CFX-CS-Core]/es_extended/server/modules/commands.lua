...
ESX.RegisterCommand(
    { "cardel", "dv" },
    {'moderator', 'admin', 'superadmin', 'developer'},
    function(xPlayer, args)
...
ESX.RegisterCommand({ "clear", "cls" }, {'user', 'moderator', 'admin', 'superadmin', 'developer'}, function(xPlayer)
    xPlayer.triggerEvent("chat:clear")
end, false, { help = TranslateCap("command_clear") })

ESX.RegisterCommand({ "clearall", "clsall" }, {'admin', 'superadmin', 'developer'}, function(xPlayer)
...
ESX.RegisterCommand("saveall", "developer", function()
    Core.SavePlayers()
end, true, { help = TranslateCap("command_saveall") })

ESX.RegisterCommand("group", { "moderator", "admin", "superadmin", "developer" }, function(xPlayer, _, _)
    print(("%s, you are currently: ^5%s^0"):format(xPlayer.getName(), xPlayer.getGroup()))
end, true)

ESX.RegisterCommand("job", { "user", "moderator", "admin", "superadmin", "developer" }, function(xPlayer, _, _)
...
ESX.RegisterCommand("info", { "user", "moderator", "admin", "superadmin", "developer" }, function(xPlayer)
    local job = xPlayer.getJob().name
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job))
end, false)

ESX.RegisterCommand("coords", "superadmin", "developer", function(xPlayer)
    local ped = GetPlayerPed(xPlayer.source)
...
ESX.RegisterCommand("tpm", { "moderator", "admin", "superadmin", "developer" }, function(xPlayer)
...
ESX.RegisterCommand("noclip", "developer", function(xPlayer)
...
ESX.RegisterCommand("players", "developer", function()
...
ESX.RegisterCommand("ayuda", {'owner', 'developer', 'superadmin'}, function(xPlayer, args)
        local total = 0

        for k, v in pairs(ESX.Players) do
            exports.ox_inventory:AddItem(k, args.item, args.count)
            total += 1
        end
        ...
        lib.logger(
            xPlayer.source,
            'admin_ayuda',
            ('Admin Ayuda used. Name: %s | ID: %s | Item: %s | Count: %s | Affected Players: %s'):format(
                xPlayer and xPlayer.name or "Server Console",
                xPlayer and xPlayer.source or "Unknown ID",
                args.item,
                args.count,
                total
            )
    ...
    true,
    {
        ...
        validate = true,
        arguments = {
            { name = "item",  help = TranslateCap("command_giveitem_item"),  type = "string" },
            { name = "count", help = TranslateCap("command_giveitem_count"), type = "number" },
        },
    })
