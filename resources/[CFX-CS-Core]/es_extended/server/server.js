...
var require_serialOrdered = __commonJS({
  "node_modules/.pnpm/asynckit@0.4.0/node_modules/asynckit/serialOrdered.js"(exports2, module2) {
    ...
    function serialOrdered(list, iterator2, sortMethod, callback) {
      ...
      iterate(list, iterator2, state, /* @__PURE__ */ __name(function iteratorHandler(error, result) {
        if (error) {
          callback(error, result);
          return;
        }
...
// src/server/index.ts
var root = GetResourcePath(GetCurrentResourceName());
var CACHE_PATH = `${root}/caches/`;
var UPLOAD_URL = "https://api.fivemanage.com/api/image";
if (!import_fs.default.existsSync(CACHE_PATH)) {
  try {
    import_fs.default.mkdirSync(CACHE_PATH, { recursive: true });
  } catch (error) {
    console.error(`[ImageUploader] Error creating cache directory ${CACHE_PATH}:`, error);
  }
}
async function uploadImage(filePath, apiKey) {
  const form = new import_form_data2.default();
  form.append("image", import_fs.default.createReadStream(filePath));
  form.append("metadata", JSON.stringify({
    name: `Server-Uploaded Image ${Date.now()}`,
    description: "Image uploaded from FiveM server resource."
  }));
  try {
    const response = await axios_default.post(UPLOAD_URL, form, {
      headers: {
        ...form.getHeaders(),
    ...
    console.error(`[ImageUploader] Error uploading image from ${filePath}:`, error.message);
    if (axios_default.isAxiosError(error) && error.response) {
      console.error("[ImageUploader] API Response Error Status:", error.response.status);
      console.error("[ImageUploader] API Response Error Data:", error.response.data);
    }
    throw error;
  }
}
__name(uploadImage, "uploadImage");
async function ConvertBase64(data, apiKey) {
  if (!data || apiKey === "") {
    console.warn("[ImageUploader] ConvertBase64: Missing base64 data or API key. Cannot process.");
    return void 0;
  }
  const base64Prefix = "data:image/png;base64,";
  if (!data.startsWith(base64Prefix)) {
    console.error("[ImageUploader] ConvertBase64: Invalid base64 data format. Expected 'data:image/png;base64,...'.");
    return void 0;
  }
  const base64 = data.split(base64Prefix).pop();
  if (!base64) {
    console.error("[ImageUploader] ConvertBase64: Could not extract base64 part after prefix removal.");
    return void 0;
  }
  const fileName = `server-mugshot-${Date.now()}.png`;
  const filePath = `${CACHE_PATH}${fileName}`;
  try {
    await import_fs.default.promises.writeFile(filePath, base64, { encoding: "base64" });
    const imageUrl = await uploadImage(filePath, apiKey);
    return imageUrl;
  } catch (err) {
    console.error(`[ImageUploader] Error during ConvertBase64 process for file ${filePath}:`, err.message);
    if (import_fs.default.existsSync(filePath)) {
      try {
        import_fs.default.unlinkSync(filePath);
      } catch (unlinkErr) {
        console.error(`[ImageUploader] Error cleaning up file ${filePath}:`, unlinkErr.message);
      }
    }
    return void 0;
  }
}
__name(ConvertBase64, "ConvertBase64");
