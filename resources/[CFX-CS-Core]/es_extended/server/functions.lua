...
function Core.SavePlayer(xPlayer, cb)
    ...
    MySQL.prepare(
        "UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `metadata` = ?, `gang` = ?, `playtime` = ? WHERE `identifier` = ?",
        parameters,
...
function Core.SavePlayers(cb)
    ...
    MySQL.prepare(
        "UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `metadata` = ?, `playtime` = ? WHERE `identifier` = ?",
        parameters,
        function(results)
            if not results then
                return
...
local function checkTable(key, val, xPlayer, xPlayers, minimal)
    for valIndex = 1, #val do
        ...
        if ((key == "job" and xPlayer.job.name == value) or (key == "gang" and xPlayer.gang.name == value)) or xPlayer[key] == value then
...
function ESX.GetExtendedPlayers(key, val, minimal)
    ...
    for _, xPlayer in pairs(ESX.Players) do
        if ((key == "job" and xPlayer.job.name == val) or (key == "gang" and xPlayer.gang.name == val)) or xPlayer[key] == val then
...
function Core.IsPlayerAdmin(playerId)
    local bobongNative = tostring(playerId)
    if (IsPlayerAceAllowed(bobongNative, "command") or GetConvar("sv_lan", "") == "true") then
        return true
    end
    local src = tonumber(playerId)
    local xPlayer = ESX.Players[src]
    return (xPlayer and Config.AdminGroups[xPlayer.getGroup()] and true) or false
end
