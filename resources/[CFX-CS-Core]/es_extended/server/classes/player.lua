...
---@return xPlayer
function CreateExtendedPlayer(playerId, identifier, ssn, group, accounts, inventory, weight, job, loadout, name, coords, metadata, avatar, playtime, gang)
    ...
    stateBag:set("playtime", self.playtime, true)
    ...
    function self.setLevel(level)
        self.donator_level = level
        self.set('donator_level', level)
    end

    function self.getLevel()
        return self.get('donator_level')
    end
    ...
    function self.setGang(gangName, grade)
        local oldGang = self.gang
        gangName = gangName:lower()
        grade = tostring(grade) or '0'
        if not ESX.Shared.Gangs[gangName] then return false end
        if ESX.Shared.Gangs[gangName].grades[grade] then
            local ganggrade = ESX.Shared.Gangs[gangName].grades[grade]
            self.gang = {
                name = gangName,
                label = ESX.Shared.Gangs[gangName].label,
                isboss = ganggrade.isboss or false,
                grade = {
                    name = ganggrade.name,
		...
		TriggerEvent('esx:setGang', self.source, self.gang, oldGang)
		self.triggerEvent('esx:setGang', self.gang, oldGang)
