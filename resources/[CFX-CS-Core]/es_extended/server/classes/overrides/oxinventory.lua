...
Core.PlayerFunctionOverrides.OxInventory = {
    ...
    addAccountMoney = function(self)
        return function(accountName, money, reason)
            ...
            if Inventory.accounts[accountName] then
                local invkongResource = GetInvokingResource()
                if invkongResource then
                    if reason then
                        lib.logger(self.source, 'account-log', ('%s added %sx %s to %s reason: %s'):format(invkongResource, money, accountName, self.name, reason))
                    else
                        lib.logger(self.source, 'account-log', ('%s added %sx %s to %s'):format(GetCurrentResourceName(), money, accountName, self.name))
                    end
                end
