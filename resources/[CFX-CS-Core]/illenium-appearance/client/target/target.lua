...
RegisterCommand('refreshclothingstore', function()
    if Config.EnablePedsForShops and next(Config.Stores) then
        if not next(TargetPeds.Store) then
            RemoveTargets()
            SetupStoreTargets()
            print('refreshing missing target ped store')
        else
            print('the Store table is not empty')
        end
        for i = 1, #TargetPeds.Store do
            if not DoesEntityExist(TargetPeds.Store[i]) then
                RemoveTargets()
                SetupStoreTargets()
                print('refreshing missing target ped store')
                break
    ...
    if Config.EnablePedsForClothingRooms and next(Config.ClothingRooms) then
        if not next(TargetPeds.ClothingRoom) then
            RemoveTargets()
            SetupClothingRoomTargets()
            print('refreshing missing target ped clothing room')
        else
            print('the ClothingRoom table is not empty')
        end
        for i = 1, #TargetPeds.ClothingRoom do
            if not DoesEntityExist(TargetPeds.ClothingRoom[i]) then
                RemoveTargets()
                SetupClothingRoomTargets()
                print('refreshing missing target ped clothing room')
                break
    ...
    if Config.EnablePedsForPlayerOutfitRooms and next(Config.PlayerOutfitRooms) then
        if not next(TargetPeds.PlayerOutfitRoom) then
            RemoveTargets()
            SetupPlayerOutfitRoomTargets()
            print('refreshing missing target player outfit room')
        else
            print('the PlayerOutfitRoom table is not empty')
        end
        for i = 1, #TargetPeds.PlayerOutfitRoom do
            if not DoesEntityExist(TargetPeds.PlayerOutfitRoom[i]) then
                RemoveTargets()
                SetupPlayerOutfitRoomTargets()
                print('refreshing missing target player outfit room')
                break
