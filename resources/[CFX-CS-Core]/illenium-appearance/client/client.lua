...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    client.startPlayerCustomization(function(appearance)
        if (appearance) then
            TriggerServerEvent("illenium-appearance:server:saveAppearance", appearance, exports['illenium-appearance']:IsNew())
            if onSubmit then
                onSubmit()
            end
...
local function OpenClothingShop(data)
    local config = GetDefaultConfig()
    ...
    if data.isPedMenu then
        ...
        config.headOverlays = data.isDeveloper and true or false
        config.tattoos = not Config.RCoreTattoosCompatibility and false
    end
    OpenShop(config, data.isPedMenu, "clothing")
end
...
function OpenMenu(isPedMenu, menuType, menuData, isDeveloper)
    ...
    if menuType == "default" then
        ...
        menuItems[#menuItems + 1] = {
            ...
            args = {isPedMenu = isPedMenu, isDeveloper = isDeveloper}
        }
...
RegisterNetEvent("illenium-appearance:client:openClothingShopMenu", function(isPedMenu)
    ...
    OpenMenu(isPedMenu, "default", nil, false)
end)
RegisterNetEvent("illenium-appearance:client:openClothingShopMenu2", function(isPedMenu)
    if source ~= 65535 then return end
    if type(isPedMenu) == "table" then
        isPedMenu = false
    end
    OpenMenu(isPedMenu, "default", nil, true)
end)
