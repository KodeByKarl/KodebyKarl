...
RegisterServerEvent("illenium-appearance:server:saveAppearance", function(appearance, isNew)
    local src = source
    ...
    if isNew then
        lib.callback('illenium-appearance:check', src, function(new)
            if new then
...
lib.addCommand("pedadmin", {
    ...
    restricted = {'group.superadmin', 'group.developer'}
}, function(source, args)
    local target = source
    if args.playerID then
        local citizenID = Framework.GetPlayerID(args.playerID)
    ...
    TriggerClientEvent("illenium-appearance:client:openClothingShopMenu2", target, true)
end)
...
lib.addCommand("reloadskin", { help = _L("commands.reloadskin.title"), restricted = 'group.developer'}, function(source)
...
RegisterServerEvent("illenium-appearance:server:saveTargetAppearance", function(appearance, targetPlayer)
    local target = targetPlayer
    local src = source
    if target == -1 then return end
    local xTarget = Framework.GetPlayerFromId(target)
    local xPlayer = Framework.GetPlayerFromId(src)
    local dist = #(xPlayer.getCoords(true) - xTarget.getCoords(true))
    if dist <= 3.0 and appearance ~= nil then
        Framework.SaveAppearance(appearance, xTarget.identifier)
    end
end)