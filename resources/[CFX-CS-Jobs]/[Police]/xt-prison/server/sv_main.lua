...
local function savePlayerJailTime(src, returnInv)
    ...
    if returnInv ~= false and confiscated[src] then
...
local function loadPlayerJailTime(src)
    if isChecked[src] then return end
    isChecked[src] = true
...
RegisterNetEvent('xt-prison:server:persistJailTime', function()
    savePlayerJailTime(source, false)
end)
...
RegisterNetEvent('xt-prison:server:removeItems', function()
    ...
    if next(playerItems) and not next(confiscatedItems) then -- Checks if player has items and confiscated table is empty
        ...
        if PrisonLogs and PrisonLogs.Items then
            local count = 0
            for _ in pairs(playerItems) do count += 1 end
            PrisonLogs.Items({
                action = 'confiscate',
                src = src,
    ...
    for k,v in pairs(config.tempItems) do
        ox_inventory:AddItem(src, k, v)
    end
...
RegisterNetEvent('xt-prison:server:returnItems', function()
    local src = source
    local cid = getCharID(src)
    if Player(src).state.jailTime > 0 and Player(src).state.inJail then
