...
function Framework.Client.ProgressBar(text, duration, anim, prop, cb, cbCancelled)
  -- Never pass empty prop/anim tables: ox_lib treats {} as a prop and calls requestModel(nil)
  local animData = (anim and anim.dict and anim.name) and {
    dict = anim.dict,
    clip = anim.name
  } or nil

  local propData = (prop and prop.model) and {
    model = type(prop.model) == "number" and prop.model or joaat(prop.model),
    bone = prop.bone,
    pos = prop.coords,
    rot = prop.rotation
  } or nil
  ...
  elseif Config.ProgressBar == "qb" then
    QBCore.Functions.Progressbar(text or "", text, duration, false, true, {
      ...
      bone = propData.bone,
      coords = prop and prop.coords,
      rotation = prop and prop.rotation
    } or {}, {}, cb(), cbCancelled and cbCancelled() or false)
  end
end
...
function Framework.Client.RepairVehicle(vehicle)
  pcall(function()
    lib.callback.await('jg-mechanic:server:fg-repair', false)
  end)
...
function Framework.Client.GetBalance(type)
  ...
  elseif Config.Framework == "ESX" then
    ...
    for _, acc in pairs(ESX.GetPlayerData().accounts) do
