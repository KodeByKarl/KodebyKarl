...
local lastNuiPayload = nil
local hudOwnsMileageUi = false

local function sendToNui(data)
  if GetResourceState("jg-hud") == "started" then
    if not hudOwnsMileageUi then
      SendNUIMessage({ type = "hide" })
      hudOwnsMileageUi = true
      lastNuiPayload = nil
    end
    return
  end

  hudOwnsMileageUi = false

  if Config.ShowMileage then
    local key = data.type .. ":" .. tostring(data.value) .. ":" .. tostring(data.unit)
    if lastNuiPayload == key then return end
    lastNuiPayload = key
    SendNUIMessage(data)
  end
end
