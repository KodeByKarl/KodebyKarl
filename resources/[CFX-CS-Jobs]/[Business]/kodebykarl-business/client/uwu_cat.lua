...
CreateThread(function()
	for k, v in pairs(PedList) do
		if v.clickable then
			exports['qtarget']:AddBoxZone("CatPat"..k, vector3(v.coords.x, v.coords.y, v.coords.z-0.3), 0.7, 0.7, { name="CatPat"..k, heading = 0, debugPoly=false, minZ= v.coords.z-1.5, maxZ=v.coords.z-0.5 }, 
				{ options = { { event = "jim-catcafe:CatPat", icon = "fas fa-paw", label = "Pet the Cat" }, },
				  distance = 1.5
			})
		end
	end
...
RegisterNetEvent('jim-catcafe:CatPat', function()
	...
	for k,v in pairs (shopPeds) do
		...
		if dist < 2 then
			...
			if ppCoords.z > 22 then 
				...
				TaskPlayAnim(pid, 'anim@amb@clubhouse@tutorial@bkr_tut_ig3@', 'machinic_loop_mechandplayer', 24.0, 16.0, -1, 49, 0, 0, 0, 0)
			end
...
function nearPed(model, coords, heading, gender, animDict, animName, scenario)
	...
	ped = CreatePed(5, model, x, y, z - 1, heading, false, true)
...
PedList = { 
	...
	{ coords = vector3(-583.62, -1069.51, 22.99), heading = 0, animDict = "creatures@cat@amb@world_cat_sleeping_ground@base", animName = "base", clickable = true, frozen = true },
...
AddEventHandler('onResourceStop', function(r) if r == GetCurrentResourceName() then for k, v in pairs(PedList) do exports['qtarget']:RemoveZone("CatPat"..k) end for k, v in pairs(shopPeds) do DeletePed(v) end end end)