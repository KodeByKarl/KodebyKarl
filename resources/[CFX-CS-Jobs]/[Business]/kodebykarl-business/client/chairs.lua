...
Citizen.CreateThread(function()
    for k, v in pairs(chairs) do
        Chairs['CatChair' .. k] =
            exports['qtarget']:AddBoxZone('CatChair' .. k, v.location, v.width, v.height, {name = 'CatChair' .. k, heading = v.heading, debugPoly = false, minZ = v.minZ, maxZ = v.maxZ, },
                {options = {{event = "w_uwucafe:chair", icon = 'fas fa-chair', label = 'Sit Down', loc = v.location, head = v.heading, seat = v.seat}, },
                    distance = v.distance
                })
    end
end)

RegisterNetEvent('w_uwucafe:chair', function(data)
    local canSit = true
    local sitting = false
    if canSit then
...
AddEventHandler('onResourceStop', function(r) if r == GetCurrentResourceName() then for k, v in pairs(Chairs) do exports['qtarget']:RemoveZone(k) end end end)
