...
export default function App() {
  ...
  useEffect(() => {
    const handleFiveMMessage = (e: MessageEvent) => {
      ...
      const eventName = data.eventName || data.action;
      ...
      if (eventName === "initFunctionInvoking" && data.count) {
        const pct = Math.round(((Number(data.idx) || 0) / Number(data.count)) * 100);
        setProgress(Math.max(0, Math.min(100, pct)));
        if (data.name) setCurrentStepMessage(`Loading ${data.name}...`);
        return;
      }

      if (eventName === "startInitFunction" || eventName === "startInitFunctionOrder" || eventName === "startDataFileInit") {
        if (data.type) setCurrentStepMessage(`Loading ${data.type}...`);
        else if (data.name) setCurrentStepMessage(`Loading ${data.name}...`);
        return;
      }

      if (eventName === "performMapLoadFunction") {
        setCurrentStepMessage("Loading map elements...");
        return;
      }

      if (eventName === "onLogLine" && data.message) {
        setCurrentStepMessage(String(data.message));
      }
    };
