local vehdamage = require 'configs.vehdamage'
if not vehdamage.enable then return end
...
local storedEngineHealth = {}
local storedBodyHealth = {}
math.randomseed(GetGameTimer());

local tireBurstMaxNumber = vehdamage.randomTireBurstInterval * 1200; 										-- the tire burst lottery runs roughly 1200 times per minute
if vehdamage.randomTireBurstInterval ~= 0 then tireBurstLuckyNumber = math.random(tireBurstMaxNumber) end	-- If we hit this number again randomly, a tire will burst.
...
exports('isPedDrivingAVehicle', isPedDrivingAVehicle)
...
if vehdamage.torqueMultiplierEnabled or vehdamage.preventVehicleFlip or vehdamage.limpMode then
	CreateThread(function()
		while true do
			Wait(1)
			if vehdamage.torqueMultiplierEnabled or vehdamage.sundayDriver or vehdamage.limpMode then
				if pedInSameVehicleLast then
					local factor = 1.0
					if vehdamage.torqueMultiplierEnabled and healthEngineNew < 900 then
						factor = (healthEngineNew+200.0) / 1100
					end
					if vehdamage.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if accelerator > 127 then
								...
								local acc = fscale(accelerator, 127.0, 254.0, 0.1, 1.0, 10.0-(vehdamage.sundayDriverAcceleratorCurve*2.0))
								factor = factor * acc
							end
							if brake > 127 then
								-- Forward and braking
								isBrakingForward = true
								brk = fscale(brake, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(vehdamage.sundayDriverBrakeCurve*2.0))
							end
						elseif speed <= -1.0 then
							-- Going reverse
							if brake > 127 then
								-- Reversing and accelerating (using the brake)
								local rev = fscale(brake, 127.0, 254.0, 0.1, 1.0, 10.0-(vehdamage.sundayDriverAcceleratorCurve*2.0))
								factor = factor * rev
							end
							if accelerator > 127 then
								-- Reversing and braking (Using the accelerator)
								isBrakingReverse = true
								brk = fscale(accelerator, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(vehdamage.sundayDriverBrakeCurve*2.0))
							end
						else
					...
					if vehdamage.limpMode == true and healthEngineNew < vehdamage.engineSafeGuard + 5 then
						factor = vehdamage.limpModeMultiplier
					end
					SetVehicleEngineTorqueMultiplier(vehicle, factor)
				end
			end
			if vehdamage.preventVehicleFlip then
...
CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			...
			healthEngineDeltaScaled = healthEngineDelta * vehdamage.damageFactorEngine * vehdamage.classDamageMultiplier[vehicleClass]

			healthBodyCurrent = GetVehicleBodyHealth(vehicle)
			if healthBodyCurrent == 1000 then healthBodyLast = 1000.0 end
			healthBodyNew = healthBodyCurrent
			healthBodyDelta = healthBodyLast - healthBodyCurrent
			healthBodyDeltaScaled = healthBodyDelta * vehdamage.damageFactorBody * vehdamage.classDamageMultiplier[vehicleClass]

			healthPetrolTankCurrent = GetVehiclePetrolTankHealth(vehicle)
			if vehdamage.compatibilityMode and healthPetrolTankCurrent < 1 then
			...
			healthPetrolTankDeltaScaled = healthPetrolTankDelta * vehdamage.damageFactorPetrolTank * vehdamage.classDamageMultiplier[vehicleClass]

			if healthEngineCurrent > vehdamage.engineSafeGuard+1 then
				SetVehicleUndriveable(vehicle,false)
			end

			if healthEngineCurrent <= vehdamage.engineSafeGuard+1 and vehdamage.limpMode == false then
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					if healthEngineCombinedDelta > (healthEngineCurrent - vehdamage.engineSafeGuard) then
					...
					if healthEngineCombinedDelta > healthEngineCurrent then
						healthEngineCombinedDelta = healthEngineCurrent - (vehdamage.cascadingFailureThreshold / 5)
					end
					...
					if healthEngineNew > (vehdamage.cascadingFailureThreshold + 5) and healthEngineNew < vehdamage.degradingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.038 * vehdamage.degradingHealthSpeedFactor)
					end

					-- If Damage is near catastrophic, cascade the failure
					if healthEngineNew < vehdamage.cascadingFailureThreshold then
						healthEngineNew = healthEngineNew-(0.1 * vehdamage.cascadingFailureSpeedFactor)
					end

					-- Prevent Engine going to or below zero. Ensures you can reenter a damaged car.
					if healthEngineNew < vehdamage.engineSafeGuard then
						healthEngineNew = vehdamage.engineSafeGuard
					end

					-- Prevent Explosions
					if vehdamage.compatibilityMode == false and healthPetrolTankCurrent < 750 then
				...
				local newFDeformationDamageMult = fDeformationDamageMult ^ vehdamage.deformationExponent	-- Pull the handling file value closer to 1
				if vehdamage.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fDeformationDamageMult', newFDeformationDamageMult * vehdamage.deformationMultiplier) end  -- Multiply by our factor
				if vehdamage.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(vehicle, 'CHandlingData', 'fWeaponDamageMult', vehdamage.weaponsDamageMultiplier/vehdamage.damageFactorBody) end -- Set weaponsDamageMultiplier and compensate for damageFactorBody
				...
				local newFCollisionDamageMultiplier = fCollisionDamageMult ^ vehdamage.collisionDamageExponent	-- Pull the handling file value closer to 1
				...
				--Modify it by pulling all number a towards 1.0
				local newFEngineDamageMult = fEngineDamageMult ^ vehdamage.engineDamageExponent	-- Pull the handling file value closer to 1
				...
				if healthBodyCurrent < vehdamage.cascadingFailureThreshold then
					healthBodyNew = vehdamage.cascadingFailureThreshold
				end
			...
			lastVehicle=vehicle
			if vehdamage.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 10 then tireBurstLottery() end
		else
			if pedInSameVehicleLast == true then
				-- We just got out of the vehicle
				lastVehicle = cache.vehicle
				if vehdamage.deformationMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fDeformationDamageMult', fDeformationDamageMult) end -- Restore deformation multiplier
				SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fBrakeForce', fBrakeForce)  -- Restore Brake Force multiplier
				if vehdamage.weaponsDamageMultiplier ~= -1 then SetVehicleHandlingFloat(lastVehicle, 'CHandlingData', 'fWeaponDamageMult', vehdamage.weaponsDamageMultiplier) end	-- Since we are out of the vehicle, we should no longer compensate for bodyDamageFactor
...
function GetEngineHealthByScript(theVehicle)
	if lastVehicle == theVehicle then
		return healthEngineLast
	end
	return false
end

function GetBodyHealthByScript(theVehicle)
	if lastVehicle == theVehicle then
		return healthBodyLast
	end
	return false
end

function SetEngineHealthByScript(theVehicle, health)
...
function SetBodyHealthByScript(theVehicle, health)
	if vehicle == theVehicle then
...
exports('SetBodyHealthByScript', SetBodyHealthByScript)
exports('SetEngineHealthByScript', SetEngineHealthByScript)
exports('GetEngineHealthByScript', GetEngineHealthByScript)
exports('GetBodyHealthByScript', GetBodyHealthByScript)