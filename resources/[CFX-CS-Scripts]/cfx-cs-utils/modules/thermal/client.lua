...
CreateThread(function()
	while true do
        local sleep = 500
        if Player(cache.serverId).state.job ~= nil and helpers.HasJob(config.jobs) then
            if IsHelicopter() then
                ...
                if IsHeliHighEnough(heli) then
                    if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
                        ...
                        helicam = true
                        TriggerServerEvent('kodebykarl-utils:thermal:heliVision', true)
                    end
                ...
                if IsControlJustPressed(0, toggle_spotlight) and cache.seat == -1 then
            ...
            if helicam then
                ...
                while helicam and not Player(cache.serverId).state.dead and (cache.vehicle == heli) and IsHeliHighEnough(heli) do
                    if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
                        PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                        helicam = false
                    end
                ...
                SetSeethrough(false)
                vision_state = 0
                TriggerServerEvent('kodebykarl-utils:thermal:heliVision', false)
            end
        end
		Wait(sleep)
	end
...
function IsHelicopter()
    for k in pairs(config.heli) do
        if IsVehicleModel(cache.vehicle, k) == 1 then
            return true
        end
    end
    return false
