...
function setVehicleDeformation(vehicle, deformation)
  ...
  if type(deformationPoints) ~= "table" then return end

  deformGeneration[vehicle] = (deformGeneration[vehicle] or 0) + 1
  local myGeneration = deformGeneration[vehicle]
  ...
  CreateThread(function()
    ...
    while deform and iteration < MAX_DEFORM_ITERATIONS do
      if not DoesEntityExist(vehicle) or deformGeneration[vehicle] ~= myGeneration then return end
    ...
    deformGeneration[vehicle] = nil
  end)
end

-- Every client that streams this vehicle must apply dents locally.
-- SetVehicleDamage is not reliably networked, so the owner would otherwise
-- see a wreck while other players see a repaired body.
AddStateBagChangeHandler("jgAgDeform", nil, function(bagName, _, value)
  if type(value) ~= "table" then return end

  CreateThread(function()
    local entity = GetEntityFromStateBagName(bagName)
    local attempts = 0
    while (not entity or entity == 0) and attempts < 50 do
      Wait(100)
