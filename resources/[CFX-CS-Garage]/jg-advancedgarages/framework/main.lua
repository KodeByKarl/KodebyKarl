...
Framework = {
  ...
  Queries = {
    ...
    GetVehicle = "SELECT * FROM %s WHERE %s = ? AND REPLACE(UPPER(plate), ' ', '') = ?",
    GetVehicleNoIdentifier = "SELECT * FROM %s WHERE REPLACE(UPPER(plate), ' ', '') = ?",
    GetVehiclePlateOnly = "SELECT plate FROM %s WHERE REPLACE(UPPER(plate), ' ', '') = ?",
    GetPrivateGarages = "SELECT * FROM player_priv_garages WHERE owners LIKE ?",
    StoreVehicle = "UPDATE %s SET in_garage = 1, stored = 1, garage_id = ?, fuel = ?, body = ?, engine = ?, damage = ? WHERE %s IN (?) AND plate = ?",
    SetInGarage = "UPDATE %s SET in_garage = 1, stored = 1 WHERE REPLACE(UPPER(plate), ' ', '') = ?",
    UpdateProps = "UPDATE %s SET %s = ? WHERE plate = ?",
    VehicleDriveOut = "UPDATE %s SET in_garage = 0, stored = 0 WHERE %s = ? AND REPLACE(UPPER(plate), ' ', '') = ?",
