...
function Framework.Client.GetPlayerGang()
  ...
  elseif Config.Framework == "ESX" then
    local player = Framework.Client.GetPlayerData()
    local gang = player and player.gang
...
function Framework.Client.VehicleGetFuel(vehicle)
  ...
  elseif Config.FuelSystem == "ox_fuel" then
    -- ox_fuel stores the real value in the state bag; native level can stay at 100.
    ...
    if type(stateFuel) == "number" then
      -- Prefer the lower value so a stale full native/state cannot wipe real fuel
      return math.min(stateFuel + 0.0, nativeFuel)
    end
    return nativeFuel
...
--- Carlock exports live on kodebykarl-ui. `provide 'cfx-keydi-ui'` makes GetResourceState
--- report started, but exports['cfx-keydi-ui'] does not resolve to the providing resource.
local function carlockKey(give, plate)
  local ok = false
...
function Framework.Client.VehicleGiveKeys(plate, vehicleEntity, origin)
  ...
  elseif Config.VehicleKeys == "cfx-keydi-carlock" then
    carlockKey(true, plate)
